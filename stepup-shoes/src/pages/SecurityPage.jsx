...
const securityPractices = [
  {
    ...
    description: 'All data transmitted between the user\'s browser and our servers is encrypted using TLS 1.3 — the latest encryption standard. This prevents man-in-the-middle attacks and data interception.',
    color: '#4CAF50'
  },
  {
    icon: ShieldCheck,
    title: 'JWT Token Sessions',
    description: 'Stateless authentication using JSON Web Tokens with 24-hour expiry. Tokens are signed with RS256 algorithm and include user claims. Refresh tokens rotate on each use for enhanced security.',
    color: '#2196F3'
  },
  {
    icon: Clock,
    title: 'Rate Limiting',
    description: 'API endpoints are protected with rate limiting — 100 requests per 15 minutes per IP address. This prevents brute-force attacks, DDoS attempts, and API abuse.',
    color: '#FF9800'
  },
  {
    icon: Hash,
    title: 'Bcrypt Password Hashing',
    description: 'All passwords are hashed using bcrypt with 10 salt rounds before storage. Even if the database is compromised, original passwords cannot be recovered from the hashes.',
    color: '#9C27B0'
  },
  {
    icon: Shield,
  ...
  },
  {
    ...
    description: 'All user inputs are sanitized and escaped before rendering. Content Security Policy headers prevent inline script execution. HTML entities are encoded to prevent script injection.',
    color: '#F44336'
  }
];
    ...
    <main className="pt-24 pb-20 px-6 min-h-screen">
      <div className="max-w-[1280px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          ...
          >
            <h2 className="font-[family-name:var(--font-display)] text-2xl text-white tracking-wide mb-6">
            ...
            >
              ...
              <div className="mb-6 p-4 rounded-xl bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/20">
                ...
                <p className="text-xs text-[var(--color-text-secondary)]">
                  Email: <span className="text-white font-mono">user@stepup.com</span>
                </p>
                <p className="text-xs text-[var(--color-text-secondary)]">
                ...
                <button
                  onClick={quickFill}
                  className="mt-3 flex items-center gap-1.5 px-3 py-1.5 bg-[var(--color-accent)] text-white text-xs font-semibold rounded-lg hover:bg-[var(--color-accent-soft)] transition-colors"
                >
                  <Zap size={12} />
                  Quick Fill
                </button>
              ...
              {success ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-12 pr-4 py-3 bg-[var(--color-bg)] border rounded-lg text-white placeholder:text-[var(--color-text-secondary)]/50 outline-none transition-colors ${
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-12 pr-12 py-3 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg text-white placeholder:text-[var(--color-text-secondary)]/50 outline-none focus:border-[var(--color-accent)] transition-colors"
                      />
                      <button
                        type="button"
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--color-text-secondary)] hover:text-white"
                      >
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-[var(--color-accent)] text-white font-semibold rounded-lg flex items-center justify-center gap-2 hover:bg-[var(--color-accent-soft)] transition-colors disabled:opacity-70"
                  >
                    {loading ? (
                      <>
        ...
        {/* Security Practices */}
        <h2 className="font-[family-name:var(--font-display)] text-3xl text-white tracking-wide mb-8 text-center">

...
import { ShoppingBag, Crown, Handshake, Users, TrendingUp, IndianRupee, BarChart3, Target } from 'lucide-react';
import { revenueStats, revenueStreams, monthlyRevenue } from '../data/revenueStats';

const iconMap = { ShoppingBag, Crown, Handshake, Users };
      ...
      <div className="max-w-[1280px] mx-auto">
        ...
        >
          ...
          <p className="text-[var(--color-text-secondary)] mt-4 max-w-2xl mx-auto text-lg">
            How StepUp generates sustainable revenue through diversified income streams
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {[
            ...
            >
              ...
              <p className="font-[family-name:var(--font-display)] text-2xl md:text-3xl text-white">{value}</p>
        ...
        >
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-white font-semibold text-lg">Monthly Revenue Trend (₹ Lakhs)</h3>
          </div>
          <div className="flex items-end justify-between gap-4 h-48">
            {monthlyRevenue.map((item, i) => (
              <div key={item.month} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-xs text-[var(--color-text-secondary)]">₹{item.revenue}L</span>
                <motion.div
                  initial={{ height: 0 }}
                  whileInView={{ height: `${(item.revenue / 50) * 100}%` }}
                  viewport={{ once: true }}
